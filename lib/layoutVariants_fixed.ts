...
export function getLayoutVariant(slug: string): LayoutVariant {
  // Guaranteed distribution using slug-based mapping with rotation
  const variants: LayoutVariant[] = ['A', 'B', 'C', 'D', 'E']
  
  // Create a deterministic mapping from slug to variant
  const slugHash = hashString(slug)
  
  // Use different offsets for different slug characteristics to ensure variety
  const lengthOffset = (slug.length * 7) % 5
  const firstCharOffset = (slug.charCodeAt(0) * 3) % 5
  const lastCharOffset = (slug.charCodeAt(slug.length - 1) * 11) % 5
  
  // Combine offsets with hash for distribution
  const combinedIndex = (slugHash + lengthOffset + firstCharOffset + lastCharOffset) % 5
  
  // Apply rotation based on slug position in alphabet to prevent clustering
  const firstChar = slug.charAt(0).toLowerCase()
  const rotationOffset = 'abcde'.indexOf(firstChar) + 1
  
  const finalIndex = (combinedIndex + rotationOffset) % 5
  
  return variants[finalIndex]
}
...
export function getLayoutConfig(variant: LayoutVariant, slug: string = ''): LayoutConfig {
  ...
  const variationIndex = variationHash % 3
